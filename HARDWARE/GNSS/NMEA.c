...
const u32 BAUD_id[9]={4800,9600,19200,38400,57600,115200,230400,460800,921600};//模块支持波特率数组,中科微模块使用115200
//从buf里面得到第cx个逗号所在的位置
...
void NMEA_BDRMC_Analysis(nmea_msg *gpsx,u8 *buf)
{
	...
	u32 temp;	   
	float rs;  
	p1=(u8*)strstr((const char *)buf,"$BDRMC");//"$GNRMC",经常有&和GNRMC分开的情况,故只判断GPRMC.
...
{
	NMEA_BDRMC_Analysis(gpsx,buf);	//GPNMC解析
}
///////////////////////////////////////////UBLOX 配置代码/////////////////////////////////////
////检查CFG配置执行情况
////返回值:0,ACK成功
